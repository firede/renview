...
- 控件按作用域归位：单列/双列在文件工具条（仅展示原始 diff 时可见），不置于顶栏；简化与原始 diff 两视图的行高与外间距对齐（切换无位移）；投影态通过文件工具条底色可一眼识别。
- 原始 diff 的行号几何与字体以简化视图为基准对齐（44px 定宽 gutter、11px 行号、20px 行高、同一 monospace 栈）：react-diff-view 的 `.diff-line` 显式引用 `--diff-font-family` 不走继承，字体对齐须在 `:root` 覆盖该变量而非写在 `.diff` 上。
