...
- 语言 wasm 新增：tree-sitter-go 0.25.0（npm 自带 wasm）；tree-sitter-gdscript 6.1.0（npm 无 wasm，sync-wasm 经 docker emscripten/emsdk:3.1.74 + tree-sitter-cli 0.25.10 从 grammar 源码构建，wasm/gdscript.wasm 存在则跳过）；tree-sitter-python 0.25.0（npm 自带 wasm）。
